...
def sort(a_list: list[str]) -> list[str]:
    """I am converting the responses for the 'difficulty' question from str to int, then I am isolating all responses that are greater than 5."""
    rank_difficulty: list[str] = []
    new_list: list[int] = []
    for value in a_list:
        idx: int = int(value)
        new_list.append(idx)  
        if idx >= 5: 
            rank_difficulty.append(str(idx))
    return rank_difficulty